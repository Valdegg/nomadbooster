#!/usr/bin/env python3
"""
Test script for Numbeo safety data fetcher with BrightData
"""

import asyncio
import logging
import os
from numbeo_safety_score import fetch_safety_data_for_city, tool_fetch_safety_data

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

async def test_single_city():
    """Test fetching safety data for a single city"""
    try:
        print("🧪 Testing single city safety fetch...")
        
        # Test the direct function
        result = await fetch_safety_data_for_city("Lisbon")
        print(f"✅ Direct function result: {result}")
        
        # Test the MCP tool wrapper
        tool_result = await tool_fetch_safety_data({"city": "Lisbon"})
...
async def test_multiple_cities():
    """Test fetching safety data for multiple cities"""
    try:
        print("🧪 Testing multiple cities safety fetch...")
        ...
        for city in test_cities:
            try:
                ...
                print(f"✅ {city}: Crime={result.get('crime_index')}, Safety={result.get('safety_index')}")
...
async def main():
    """Main test function"""
    print("🚀 Starting Numbeo Safety Data BrightData Integration Test")
    print("=" * 60)
    ...
    if single_success and multi_success:
        print("🎉 All tests passed! Numbeo Safety BrightData integration is working.")
    elif single_success:
